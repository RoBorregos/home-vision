...
# Function to save image with unique ID
def save_image(image, folder):
    count = get_image_count(folder)
    image_id = count + 1
    cv2.imwrite(f"{folder}/image_{image_id}.jpg", image)
    print(f"Image saved as {folder}/image_{image_id}.jpg")
...
def main() :
    ...
    milk_dir = os.path.join(root_dir, "milk")
    os.makedirs(milk_dir, exist_ok=True)
    bowl_dir = os.path.join(root_dir, "bowl")
    os.makedirs(bowl_dir, exist_ok=True)
    
    cereal_dir = os.path.join(root_dir, "cereal")
    os.makedirs(cereal_dir, exist_ok=True)
    ...
    while True:
        ...
        if key == ord('m'):
            save_image(image_ocv, milk_dir)
        elif key == ord('b'):
            save_image(image_ocv, bowl_dir)
        elif key == ord('c'):
            save_image(image_ocv, cereal_dir)
        elif key == 27 or key == ord('q'):  # Escape key
              break
