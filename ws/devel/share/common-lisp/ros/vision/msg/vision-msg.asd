
(cl:in-package :asdf)

(defsystem "vision-msg"
  :depends-on (:roslisp-msg-protocol :roslisp-utils :geometry_msgs-msg
)
  :components ((:file "_package")
    (:file "img" :depends-on ("_package_img"))
    (:file "_package_img" :depends-on ("_package"))
    (:file "img_list" :depends-on ("_package_img_list"))
    (:file "_package_img_list" :depends-on ("_package"))
    (:file "level" :depends-on ("_package_level"))
    (:file "_package_level" :depends-on ("_package"))
    (:file "objectDetection" :depends-on ("_package_objectDetection"))
    (:file "_package_objectDetection" :depends-on ("_package"))
    (:file "people_count" :depends-on ("_package_people_count"))
    (:file "_package_people_count" :depends-on ("_package"))
    (:file "person" :depends-on ("_package_person"))
    (:file "_package_person" :depends-on ("_package"))
    (:file "person_list" :depends-on ("_package_person_list"))
    (:file "_package_person_list" :depends-on ("_package"))
    (:file "shelf" :depends-on ("_package_shelf"))
    (:file "_package_shelf" :depends-on ("_package"))
    (:file "target" :depends-on ("_package_target"))
    (:file "_package_target" :depends-on ("_package"))
  ))